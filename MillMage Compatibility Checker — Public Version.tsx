...
const DEFAULT_MACHINES = [
  ...
  { brand:"Various (Chinese)", model:"Machine with DSP A11/A15/A18", firmware:"other_builtin", controller:"RichAuto DSP", compatLevel:"cam", notes:"RichAuto DSP controller. CAM Only — use LinuxCNC flavor. Requires metric units. Needs to be manually set to accept feed and speed commands from G-Code in the controller." },
  { brand:"Various (Chinese)", model:"Machine with DSP 0501", firmware:"other_builtin", controller:"DSP 0501 (DDCSV)", compatLevel:"cam", notes:"DDCS-based controller. CAM Only — use LinuxCNC G-Code flavor." },
...
function score(q,m){
  ...
  if(!qn)return 0;if(mn===qn)return 100;if(mn.startsWith(qn))return 90;
  if((bn+mn).includes(qn))return 80;if(all.replace(/\s/g,"").includes(qn))return 70;
  const ws=q.toLowerCase().split(/\s+/).filter(w=>w.length>1);if(!ws.length)return 0;
  const hits=ws.filter(w=>all.includes(norm(w)));
  if(hits.length===ws.length)return 60;if(hits.length>0)return 30+(hits.length/ws.length)*25;return 0;
}
...
export default function App(){
  ...
  useEffect(()=>{
    (async()=>{
      try{
        ...
        if(r&&r.value){const p=JSON.parse(r.value);if(Array.isArray(p)&&p.length>0){setMachines(p.map((m,i)=>({...m,id:i})));setLoading(false);return;}}
      }catch(e){}
      setMachines(DEFAULT_MACHINES.map((m,i)=>({...m,id:i})));setLoading(false);
    })();
  },[]);
  ...
  return(
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif",maxWidth:900,margin:"0 auto",padding:"24px 16px"}}>
      <div style={{textAlign:"center",marginBottom:24}}>
        <h1 style={{fontSize:24,fontWeight:700,color:"#111827",margin:"0 0 6px"}}>Is my CNC compatible with MillMage?</h1>
        <p style={{fontSize:14,color:"#6b7280",margin:0}}>Search {machines.length} machines or browse the full database below.</p>
      </div>
        ...
        <div style={{animation:"fadeIn .25s ease"}}>
          <div style={{background:c.bg,border:`2px solid ${c.bdr}`,borderRadius:12,padding:"20px 24px",marginBottom:16}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
              <span style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:40,height:40,borderRadius:"50%",background:c.color,color:"#fff",fontSize:22,fontWeight:700}}>{c.icon}</span>
              <div>
                <div style={{fontSize:18,fontWeight:700,color:"#111827"}}>{sel.brand} {sel.model}</div>
                <span style={{display:"inline-block",fontSize:12,fontWeight:600,color:c.color,background:"#fff",border:`1px solid ${c.bdr}`,borderRadius:20,padding:"2px 10px",marginTop:3}}>{c.tag}</span>
              </div>
            </div>
            <p style={{fontSize:14,color:"#374151",lineHeight:1.6,margin:"0 0 14px"}}>{c.sum}</p>
          ...
          {sel.compatLevel==="full"&&<div style={{background:"#f0fdf4",borderRadius:8,padding:"12px 16px",fontSize:13,color:"#166534",marginBottom:16,lineHeight:1.5}}><strong>Ready to go!</strong> Download the <a href="https://lightburnsoftware.com/products/millmage-core" target="_blank" rel="noopener" style={{color:"#059669",fontWeight:600}}>30-day free trial</a> to verify with your setup.</div>}
          {sel.compatLevel==="cam"&&<div style={{background:"#fffbeb",borderRadius:8,padding:"12px 16px",fontSize:13,color:"#92400e",marginBottom:16,lineHeight:1.5}}><strong>You can still use MillMage!</strong> Design and export GCode, then transfer to your machine via your control software, SD card, or USB drive.</div>}
          {(sel.compatLevel==="maybe"||sel.compatLevel==="depends")&&<div style={{background:"#f5f3ff",borderRadius:8,padding:"12px 16px",fontSize:13,color:"#5b21b6",marginBottom:16,lineHeight:1.5}}><strong>Not sure?</strong> Try the <a href="https://lightburnsoftware.com/products/millmage-core" target="_blank" rel="noopener" style={{color:"#7c3aed",fontWeight:600}}>30-day free trial</a> to test with your machine.</div>}
          {sel.compatLevel==="unlikely"&&<div style={{background:"#fef2f2",borderRadius:8,padding:"12px 16px",fontSize:13,color:"#991b1b",marginBottom:16,lineHeight:1.5}}>This machine's proprietary system isn't supported. If you've upgraded the controller to GRBL/grblHAL/FluidNC, search again by your new controller type.</div>}
          <button onClick={()=>{setSel(null);setQuery("");setTimeout(()=>ref.current?.focus(),100);}} style={{display:"block",width:"100%",padding:"12px",background:"#fff",border:"1px solid #d1d5db",borderRadius:8,fontSize:14,fontWeight:500,color:"#374151",cursor:"pointer"}} onMouseEnter={e=>e.target.style.background="#f9fafb"} onMouseLeave={e=>e.target.style.background="#fff"}>← Check another machine</button>
        </div>
      ):(
        <>
          ...
          {results.length>0&&(
            <div style={{border:"2px solid #6366f1",borderTop:"1px solid #e5e7eb",borderRadius:"0 0 10px 10px",background:"#fff",overflow:"hidden",marginBottom:16,maxHeight:420,overflowY:"auto"}}>
              {results.map((m,i)=>{const cc=CC[m.compatLevel];return(
                <button key={m.id} onClick={()=>{setSel(m);setQuery("");}} style={{display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%",padding:"12px 16px",background:"transparent",border:"none",borderBottom:i<results.length-1?"1px solid #f3f4f6":"none",cursor:"pointer",textAlign:"left"}} onMouseEnter={e=>e.currentTarget.style.background="#f9fafb"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
          ...
          {query.trim().length>=2&&results.length===0&&(
            <div style={{border:"2px solid #6366f1",borderTop:"1px solid #e5e7eb",borderRadius:"0 0 10px 10px",background:"#fff",padding:"20px 16px",textAlign:"center",marginBottom:16}}>
              ...
              <p style={{fontSize:13,color:"#9ca3af",margin:0}}>Try searching by brand, model, or firmware (e.g. "GRBL", "Mach4", "LinuxCNC"). You can also browse the full database below.</p>
            </div>
          )}
          ...
          {fwHelp&&(
            <div style={{marginBottom:16,background:"#f9fafb",borderRadius:10,padding:"16px 20px",border:"1px solid #e5e7eb"}}>
              ...
              <p style={{fontSize:12,color:"#6b7280",margin:"8px 0 0",lineHeight:1.5}}>Not sure? Check with your manufacturer or try the <a href="https://lightburnsoftware.com/products/millmage-core" target="_blank" rel="noopener" style={{color:"#6366f1"}}>30-day free trial</a>.</p>
            </div>
          )}
      ...
      <div style={{marginTop:28,borderTop:"1px solid #e5e7eb",paddingTop:20}}>
        <button onClick={()=>setShowTable(!showTable)} style={{background:"none",border:"none",fontSize:16,fontWeight:700,color:"#111827",cursor:"pointer",padding:0,display:"flex",alignItems:"center",gap:8,marginBottom:showTable?16:0}}>
          <span style={{fontSize:14,transition:"transform .2s",transform:showTable?"rotate(90deg)":"rotate(0)",display:"inline-block"}}>▶</span>
          📋 Full Machine Database ({machines.length})
        </button>
        ...
        {showTable&&(
          <div>
              ...
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                ...
                <tbody>
                  ...
                  {filteredTable.map(m=>{const cc=CC[m.compatLevel];return(
                    <tr key={m.id} style={{borderBottom:"1px solid #f3f4f6",cursor:"pointer"}} onClick={()=>{setSel(m);window.scrollTo({top:0,behavior:"smooth"});}} onMouseEnter={e=>e.currentTarget.style.background="#f9fafb"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
            ...
            <p style={{fontSize:12,color:"#9ca3af",marginTop:12,lineHeight:1.5}}>Showing {filteredTable.length} of {machines.length} machines. Click any row to see full details. Machine missing? Check by firmware type in the search above.</p>
          </div>
        )}
      </div>
