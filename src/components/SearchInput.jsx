  ...
  return (
    <>
      <div style={{ position: "relative", marginBottom: results.length > 0 ? 0 : 16 }}>
        <div style={{ display: "flex", alignItems: "center", border: "2px solid " + (query.length > 0 ? theme.brand.primary : "#d1d5db"), borderRadius: results.length > 0 ? "10px 10px 0 0" : "10px", padding: "0 14px", background: "#fff", transition: "border-color .15s" }}>
      ...
      {results.length > 0 && (
        <div style={{ border: `2px solid ${theme.brand.primary}`, borderTop: "1px solid #e5e7eb", borderRadius: "0 0 10px 10px", background: "#fff", overflow: "hidden", marginBottom: 16, maxHeight: 420, overflowY: "auto" }}>
      ...
      {query.trim().length >= 2 && results.length === 0 && (
        <div style={{ border: `2px solid ${theme.brand.primary}`, borderTop: "1px solid #e5e7eb", borderRadius: "0 0 10px 10px", background: "#fff", padding: "20px 16px", textAlign: "center", marginBottom: 16 }}>
      ...
      <button onClick={() => setFwHelp(!fwHelp)} style={{ background: "none", border: "none", color: theme.brand.primary, fontSize: 14, fontWeight: 500, cursor: "pointer", padding: 0, textDecoration: "underline", textUnderlineOffset: 2, marginBottom: 12, display: "block" }}>{fwHelp ? "Hide firmware info \u2191" : "Don't know your firmware? \u2193"}</button>
      {fwHelp && (
